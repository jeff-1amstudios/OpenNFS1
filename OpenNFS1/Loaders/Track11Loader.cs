...
namespace NeedForSpeed.Loaders
{
	class Track11Loader : ITrackLoader
	{
        ...
        public void LoadTerrain(BinaryReader reader, List<TerrainSegment> terrainSegments, List<TerrainRow> terrainRows, List<PhysicalRoadNode> roadNodes)
		{
			...
			while (true)
			{
                ...
                {
                    row.RoadNode = roadNodes[segmentNbr];
		...
		private TerrainRow ReadTerrainRow(BinaryReader reader)
		{
			...
			row.Points.Add(new Vector3(reader.ReadInt16(), reader.ReadInt16(), -reader.ReadInt16()) * TrackAssembler.ScaleFactor);
			row.Points.Add(new Vector3(reader.ReadInt16(), reader.ReadInt16(), -reader.ReadInt16()) * TrackAssembler.ScaleFactor);
            row.Points.Add(new Vector3(reader.ReadInt16(), reader.ReadInt16(), -reader.ReadInt16()) * TrackAssembler.ScaleFactor);
            row.Points.Add(new Vector3(reader.ReadInt16(), reader.ReadInt16(), -reader.ReadInt16()) * TrackAssembler.ScaleFactor);
            row.Points.Add(new Vector3(reader.ReadInt16(), reader.ReadInt16(), -reader.ReadInt16()) * TrackAssembler.ScaleFactor);
            ...
            for (int i = 6; i < row.Points.Count; i++)
            {
                ...
                //row.Points[6] += new Vector3(0, 0.1f, 0);
